...
function script_usage() {
    cat << EOF
Builds & updates docker container being used to run Dagster deployment europe_belgium k8s cluster

Dependencies: gcloud, kubectl, helm, docker
Usage:
...
function main() {
    ...
    check_binary helm "halt-if-not-exists"
